...
//  WorldScramble
//
//  Created by joao camargo on 27/05/21.
//
...
struct ContentView: View {
    ...
    func addNewWord(){
        ...
        guard isOriginal(word: answer) else {
            wordError(title: "Word already in use", message: "Be original")
            return
        }
        ...
        guard isReal(word: answer) else {
            wordError(title: "This is not a real word", message: "Word does not exists")
            return
        }
    ...
    func startGame() {
        if let startWordsURL = Bundle.main.url(forResource: "start", withExtension: "txt"){
            if let startWords = try? String(contentsOf: startWordsURL) {
                ...
                rootWord = allWords.randomElement() ?? "silkform"
                return
            }
        }
